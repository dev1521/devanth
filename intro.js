/**
 * DEVANTH — Cinematic Logo Ident (Unified Intro Engine)
...
(function () {
  ...
  // DOM Elements scoped strictly to intro
  const introLayer = document.getElementById('devanth-intro');
  const container = document.getElementById('identContainer');
  ...
  function renderFrame(elapsed) {
    ...
    // 4.5s+     : Final Logo Hold & Sheen (~4.1 - 4.75s)
    // 5.2s+     : Cinematic Hold Complete -> Dispatches devanthIntroComplete
    // -----------------------------------------------------------------------
    ...
    if (elapsed >= 3.4) {
      ...
      if (elapsed < 4.1) {
        ...
        // Full hero presence
        wordmarkAlpha = 1.0;
        // Restrained single-pass specular highlight across wordmark (4.1s to 4.75s)
  ...
  function runChoreography(timestamp) {
    ...
    if (elapsed >= 5.2 && !hasCompleted) {
      hasCompleted = true;
      window.dispatchEvent(new CustomEvent('devanthIntroComplete'));
    }
  ...
  window.replayDevanthIntro = function () {
    ...
    if (portfolioEl) {
      portfolioEl.classList.remove('portfolio-visible');
      portfolioEl.classList.add('portfolio-hidden');
    }
  ...
  // Gracefully pause animation loop after intro fades out to save 100% CPU/GPU
  window.addEventListener('devanthIntroComplete', () => {
    setTimeout(() => {
      if (hasCompleted && animFrameId) {
        cancelAnimationFrame(animFrameId);
        animFrameId = null;
        isRunning = false;
      }
  ...
  // Sound toggle button in intro controls dock
  if (soundToggleBtn) {
  ...
  window.addEventListener('keydown', (e) => {
    // Only process Space/R/M if intro is visible/active
    const isIntroActive = !document.body.classList.contains('intro-complete');
    if (isIntroActive) {
